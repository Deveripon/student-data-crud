...
StudentAddingForm.onsubmit = (e) => {
  ...
  if (uniqueEmail(email)) {
    massgaeArea.innerHTML = WarnalertMassage("This email adress already exists");
    return;
  }
  // check unique mobile number
  if (uniqueMobile(mobile)) {
    massgaeArea.innerHTML = WarnalertMassage("This mobile number already exists");
    return;
  }
  ...
  if (!f_name || !l_name || !fother_name || !mother_name || !dob || !email || !address || !mobile) {
    ...
    massgaeArea.innerHTML = successalertMassage("Student data added successfully");
    e.target.reset();
    showData();
  }
  ...
  if (lsData.length > 0) {
    lsData.reverse().map((student, index) => {
      ...
      <tr class="border transform duration-300 hover:bg-slate-400 border-sky-200">
                <td>${index + 1} <span class="font-extrabold text-gray-800">.</span></td>
                <td class="pl-5"><img class="w-12 h-12" src="${student.image}" alt=""></td>
                ...
                <td> <i class="fa-solid fa-circle" style="color: #1ba73e;"></i> ${student.status}</td>
                <td>
                  <ul class="flex justify-center items-center action-button">
                    <li>
                    <a class="view-button" href="#" onclick="viewStudent('${student.studentId}')">
                    <i class="ri-eye-line text-green-700 transform hover:text-gray-800 duration-300"></i>
                    </a>
                    </li>
                    <li> 
                    <a href="#" class="edit-button" onclick="editStudent('${student.studentId}')" >
                    <i class="ri-edit-box-line text-orange-400 transform hover:text-gray-800 duration-300"></i>
                    </a>
                    </li>
                    <li>
                    <a href="#" class="delete-button" onclick="deleteStudent('${student.studentId}')" ><i
...
function editStudent(id) {
  ...
  studenstId.value = filterdData.studentId
  image.value = filterdData.image
  //set gender selection
  if (gender.value == filterdData.gender) {
    const gender = editForm.querySelector(`input[value="${filterdData.gender}"]`).checked = true;
  }
  ...
  editForm.onsubmit = (e) => {
    ...
    //add validation
    if (!objectData.f_name || !objectData.l_name || !objectData.fother_name || !objectData.mother_name || !objectData.dob || !objectData.email || !objectData.address || !objectData.mobile) {
      alerta.innerHTML = WarnalertMassage("All fields are required . Please fill up all fields");
...
function viewStudent(id) {
  ...
  const lsData = getData("students")
  const filterdData = lsData.filter((item) => item.id === id);
